from clientScrapySystem.DataFilterSystem.template.keyArr import KEY_ARR
class FilterData:
    """
    过滤器方法集合。过滤数据。将数据放入到方法中，如果被过滤则返回None。
    """
    @staticmethod
    ...
    @staticmethod
    def filterFunction_key_words(data):
        """
        过滤产品的关键字。
        """
        if not data:
            return None
        products = KEY_ARR.getKeyArr()#获得
        for product in products:
            if data['公司名'] != None:
                if product in data['公司名']:
                    data['关键字']=product
